...
//    manager->SetDeltaOneStep(config.getDouble("extMonFNAL."+volNameSuffix+".magnet.deltaOneStep", manager->GetDeltaOneStep()));
    ...
    for(unsigned i = 0, wpoff=1; i < modulePars.size(); ++i) {
      wpoff += constructChamberModule(i, wpoff, modulePars[i], hous);
    }
  ...
  mu2e::VolumeInfo MuCapWorld::constructDetectorEnclosure(const VolumeInfo& parent, const fhicl::ParameterSet& pset) {
    TubsParams wall_params(pset.get<double>("hous_radius") - pset.get<double>("hous_mantle"),
                           pset.get<double>("hous_radius"),
                           pset.get<double>("hous_length")/2+pset.get<double>("hous_cover"));

    VolumeInfo hswl = nestTubs("HSWL",
                               wall_params,
    ...
    TubsParams hous_params(0./* rIn */,
                           pset.get<double>("hous_radius") - pset.get<double>("hous_mantle"),
                           pset.get<double>("hous_length")/2);

    VolumeInfo hous = nestTubs("HOUS",
                               hous_params,
                               findMaterialOrThrow(pset.get<string>("hous_material")),
                               0, // no rotation
                               CLHEP::Hep3Vector(0,0,0), // center in parent
                               parent,
                               0,
    ...
    // FIXME: sort out what goes where.  For now just add the mg/cm2 materials

    //----------------------------------------------------------------
    // Scintillators
    nestTubs("muScint",
             TubsParams(0., 30.*CLHEP::mm, 239.*CLHEP::um/2),
             findMaterialOrThrow("G4_MYLAR"),
             0, // no rotation
             CLHEP::Hep3Vector(0,0,-850*CLHEP::mm),
             parent,
             0,
             true,
             G4Colour::Red(),
    ...
    nestTubs("t0Scint",
             TubsParams(30.*CLHEP::mm, 185*CLHEP::mm, 2*3.17499995*CLHEP::mm/2),
             findMaterialOrThrow("G4_MYLAR"),
             0, // no rotation
             CLHEP::Hep3Vector(0,0,-850*CLHEP::mm),
             parent,
    ...
    nestTubs("gabs",
             TubsParams(30.*CLHEP::mm, 185*CLHEP::mm, 200*CLHEP::mm/2),
             findMaterialOrThrow("MUCAP_GABS_GAS"),
             0, // no rotation
             CLHEP::Hep3Vector(0,0,-954*CLHEP::mm),
             parent,
             0,
             true,
             G4Colour::Yellow(),
    ...
    nestTubs("gabs_foils",
             TubsParams(0, 185*CLHEP::mm, (100 + 6)*CLHEP::um/2),
             findMaterialOrThrow("G4_MYLAR"),
             0, // no rotation
             CLHEP::Hep3Vector(0,0,-1055*CLHEP::mm),
             parent,
             0,
             true,
             G4Colour::Blue(),
